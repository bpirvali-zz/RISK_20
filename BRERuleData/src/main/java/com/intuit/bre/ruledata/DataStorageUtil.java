...
public class DataStorageUtil {
    private static Logger logger = LoggerFactory.getLogger(DataStorageUtil.class);
	...
	private static class DataStorageUtilHolder {
		...
		private static DataStorageUtil createPropertiesUtil() {
			...
			try {
				String propFile = Configuration.getJdbcPropertiesFile();
				//System.out.println("config.prop:" + propFile);
				prop.load(AppConsts.class.getResourceAsStream(propFile));
				...
				logger.error("IO Exception importing " + Configuration.getJdbcPropertiesFile(), e);
			}
			return dsUtil;
		}
	}
	...
	public DBInfo getDBInfo(String dbName) {
		...
		//if (dbInfo==null)
		//	throw new RuntimeException("Data Storage Name:'" + dbName + "' is not recognized!");
